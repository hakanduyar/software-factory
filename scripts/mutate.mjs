...
const MUTATIONS = [
  ...
  {
    id: "the required-test manifest is emptied",
    edits: [[VERIFIER,
      '  "tests/workflowPolicy.test.ts",\n  "tests/knownLimitationsHonesty.test.ts",',
      ""]],
    tests: [T_WF],
    expect: "requires tests/workflowPolicy.test.ts",
  },
  {
    id: "the manifest is declared but nothing fails on a miss",
    edits: [[VERIFIER, "if (missingRequired.length > 0) {", "if (false) {"]],
    tests: [T_WF],
    expect: "refuses when a required test is missing",
  },
  {
    ...
    edits: [[VERIFIER,
      ...
      'const presentRequired = JSON.parse(readFileSync(join(REPO_ROOT, "package.json"), "utf8")).name === "software-factory" ? REQUIRED_TESTS : [];']],
    tests: [T_WF],
