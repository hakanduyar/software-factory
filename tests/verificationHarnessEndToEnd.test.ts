...
describe("TASK-017: a repository must present, compile and RUN its deliverable", () => {
   ...
   *
   * `addDeliverable` writes tests that import and use their modules. Where a
   * case DELETES a required module or excludes it from compilation, a test
   * importing it would fail to COMPILE, and the fixture would then be refused
   * for a broken build instead of by the guard under test — sibling-guard
   * masking, the finding this task has now produced fourteen times.
   */
