...
describe("TASK-017 AC-10: no entry claims a closure the clean room does not deliver", () => {
  ...
  it("pairs no closure verb with the clean room", () => {
    ...
    for (const [heading, body] of entries()) {
       ...
       * "seals", round 19 found "overcomes". There are more synonyms than there are review rounds, and a
       ...
       */
      const CLOSURE_VERB = "(?:clos(?:e|es|ed)|eliminat(?:e|es|ed)|remov(?:e|es|ed)|solv(?:e|es|ed)|fix(?:es|ed)?|resolv(?:e|es|ed)|address(?:es|ed)?|end(?:s|ed)?|obviat(?:e|es|ed)|negat(?:e|es|ed)|cur(?:e|es|ed)|prevent(?:s|ed)?|stop(?:s|ped)?|seal(?:s|ed)?|plug(?:s|ged)?|settl(?:e|es|ed)|neutralis(?:e|es|ed)|neutraliz(?:e|es|ed)|overcom(?:e|es)|overcame|surmount(?:s|ed)?|nullif(?:y|ies|ied))";
